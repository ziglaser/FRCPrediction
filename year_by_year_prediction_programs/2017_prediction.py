...
#dict_team_OPRs = {}
dict_team_OPRs = np.load('OPRs_2017.npy').item()

dict_team_ELOs = {}
ELO_csv = open('C:\\Users\\zglas\\Documents\\2017ELO.csv',"r", encoding='utf-8-sig')
for line in ELO_csv:
    line.strip()
    team, elo = line.split(",")
...
def calculate_opr(event):
    ...
    sparse_matrix = []
    stats = [[], [], [], [], [], [], [], []]
    stats_names = ['totalPoints', 'autoFuelPoints', 'autoRotorPoints', 'teleopFuelPoints', 'techFoulCount',
                   'tba_rpEarned', 'teleopRotorPoints', 'teleopTakeoffPoints']
    ...
    np.save('OPRs_2017.npy', dict_team_OPRs)
...
def Initialize_Model():
    ...
    if data_collection:
        ...
        event_list = []
        for event in tba.events("2017", keys=True):
        ...
        test_event_list = ['2017hop', '2017carv', '2017gal', '2017tur', '2017new', '2017roe', '2017cars', '2017tes', '2017cur', '2017dar', '2017dal', '2017arc']
        ...
        np.save("X_OPR_test_2017.npy", X_OPR_test)
        np.save("X_ELO_test_2017.npy", X_ELO_test)
        np.save("X_OPR_2017.npy", X_OPR)
        np.save("X_ELO_2017.npy", X_ELO)
        np.save("Y_test_2017.npy", Y_test)
        np.save("Y_2017.npy", Y)

    X_OPR_test = np.load("X_OPR_test_2017.npy")
    X_ELO_test = np.load("X_ELO_test_2017.npy")
    X_OPR = np.load("X_OPR_2017.npy")
    X_ELO = np.load("X_ELO_2017.npy")
    Y_test = np.load("Y_test_2017.npy")
    Y = np.load("Y_2017.npy")
    ...
    OPR_model.add(Dense(7, input_dim = 16, activation='relu'))
    OPR_model.add(Dense(20, activation='relu'))
