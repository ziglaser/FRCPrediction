...
dict_team_ELOs = {}
ELO_csv = open('C:\\Users\\zglas\\Documents\\2019ELO.csv',"r")
for line in ELO_csv:
    line.strip()
    team, elo = line.split(",")
...
def calculate_opr(event):
    ...
    np.save('OPRs.npy', dict_team_OPRs)
...
def Initialize_Model():
    ...
    if data_collection:
        ...
        np.save("X_OPR_test.npy", X_OPR_test)
        np.save("X_ELO_test.npy", X_ELO_test)
        np.save("X_OPR.npy", X_OPR)
        np.save("X_ELO.npy", X_ELO)
        np.save("Y_test.npy", Y_test)
        np.save("Y.npy", Y)

    X_OPR_test = np.load("X_OPR_test.npy")
    X_ELO_test = np.load("X_ELO_test.npy")
    X_OPR = np.load("X_OPR.npy")
    X_ELO = np.load("X_ELO.npy")
